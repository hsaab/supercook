...
- **Trivial.** Work directly, verify, done. No exploration phase, no arena, no design doc, no test ceremony. A copy fix stays a copy fix.
...
Tracks skip what does not apply. An investigation stays read-only and produces an answer, not a PR. `open-pr` runs delivery only. A refactor starts by pinning current behavior with tests.
...
**The one edit worth making:** give the three arena roles three different model families. Three contestants on one model mostly agree with themselves. Three families disagree in useful ways, and the disagreement is the entire value of the arena.
...
| Worktrees | Works on the current branch, after saying so |
...
| investigation | Read-only, cited answer, no plan or tests or PR |
| refactor | Pins current behavior with characterization tests, reroutes if behavior changes |
| open-pr | Delivery only, for work that already exists in the tree |
...
skills/supercook/
  ...
  pipeline/                   one guide per phase, loaded when that phase runs
